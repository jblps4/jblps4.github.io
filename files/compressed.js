...
tileMosaicFlip = function(length,r,m,af){
	...
	if(r==0){ // if big image is front now
		...
		$id_back.animate({height:'0px',marginTop:''+margin+'px'},120,function(){
			$id.animate({height:''+height+'px',marginTop:'0px'},120);
		});	
		if(af.length>length){
			af = new Array();
			r=1;
		...
		$id.animate({height:'0px',marginTop:''+margin+'px'},120,function(){
			$id_back.animate({height:height+'px',marginTop:'0px'},120);
		});
		if(af.length>length){
			af = new Array();
			r=0;
